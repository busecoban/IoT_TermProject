...
#define TELEMETRY_PROP_NAME_DOOR "doorPosition"
...
void azure_pnp_init() {
  ...
  // Hoca bitirdi
  azure_pnp_set_telemetry_frequency(5); // Varsayılan olarak 10 saniye
  delay(1000);
}
    ...
    size_t* payload_buffer_length)
{
  ...
  doorPosition = (manyetikDurum == HIGH) ? 1 : 0;
  Serial.println( (doorPosition == 1) ? "KAPI AÇIK" : "KAPI KAPALI");
  // Hoca bitirdi
...
rc = az_json_writer_append_property_name(&jw, AZ_SPAN_FROM_STR(TELEMETRY_PROP_NAME_DOOR));
EXIT_IF_AZ_FAILED(rc, RESULT_ERROR, "Failed adding door status property name to telemetry payload.");


  //gel buraya
  // Adding door status to telemetry payload
  
  rc = az_json_writer_append_int32(&jw, doorPosition);
  EXIT_IF_AZ_FAILED(rc, RESULT_ERROR, "Failed adding door status property value to telemetry payload.");
